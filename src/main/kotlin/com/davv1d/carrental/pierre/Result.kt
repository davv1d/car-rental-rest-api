package com.davv1d.carrental.pierre
...
sealed class Result<out A> : Serializable {
    ...
    fun filter(error: RuntimeException, p: (A) -> Boolean): Result<A> = flatMap {
        if (p(it))
            this
        else
            failure(error)
    ...
    internal class Failure<out A>(internal val exception: RuntimeException) : Result<A>() {
        ...
        override fun <B> effect(onSuccess: (A) -> B, onFailure: (RuntimeException) -> Unit, onEmpty: () -> B): B {
            onFailure(exception)
            throw exception
        }
    ...
    companion object {
        ...
        operator fun <A> invoke(a: A? = null, error: RuntimeException): Result<A> = when (a) {
            null -> Failure(error)
            else -> Success(a)
        }
