...
class UserDbConditions(private val roleRepository: RoleRepository, private val userRepository: UserRepository) : ConditionGenerator<User> {
    ...
    override fun get(value: User): List<Condition<User>> {
        val con1 = Condition(value, USERNAME_EXIST_IN_DATABASE, checkFunction = { u -> userRepository.existsByUsername(u.username) })
        val con2 = Condition(value, EMAIL_EXIST_IN_DATABASE, checkFunction = { u -> userRepository.existsByEmail(u.email) })
        val con3 = Condition(value, ROLE_WITH_THIS_NAME_IS_NOT_EXIST, checkFunction = { u -> roleRepository.doesNotExistByName(u.role.name) })
